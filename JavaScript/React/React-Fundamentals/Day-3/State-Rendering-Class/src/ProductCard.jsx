...
const ProductCard = ({product,del}) => {
  return (
    <div className='w-60 h-[450px] p-2 border-2 rounded flex flex-col gap-2 justify-center items-center'>
      <div>
        <img className='h-65 object-contain' src={product.image} alt={product.title}>
        </img>
      <h1 className='font-bold'>{product.title.substring(0,23)}</h1>
      <h2 className='font-semibold'>{product.category}</h2>
      <h1 className='font-extrabold text-green-600'>₹ {product.price}</h1>
      <button className='font-extrabold bg-red-700 text-white w-fit border-2 rounded p-0.5 cursor-pointer'
