...
function initializeMonthlyChart() {
  const context = chartElements.monthlyChart.getContext("2d");
...
function updateChart() {
  ...
  if (income === 0 && expense === 0) {
    ...
    incomeExpenseChart.data.datasets[0].backgroundColor = ["#D1D5DB"];
  } else {
    incomeExpenseChart.data.labels = ["Income", "Expense"];
...
function updateCategoryChart() {
  ...
  if (Object.keys(categoryTotals).length === 0) {
    ...
    categoryChart.data.datasets[0].backgroundColor = Object.keys(
      categoryTotals,
    ).map((category) => categoryColors[category] || "#9CA3AF");
  }
...
function updateMonthlyChart() {
  ...
  monthlyChart.data.datasets[0].data = labels.map(
    (label) => monthlyData[label].income,
  );
  monthlyChart.data.datasets[1].data = labels.map(
    (label) => monthlyData[label].expense,
  );
  monthlyChart.update();
}
...
function renderExpandedChart(chart, type, title) {
  ...
  const context = chartModalElements.canvas.getContext("2d");
...
function downloadExpandedChart() {
  ...
  link.href = expandedChart.toBase64Image();

  link.click();
}

function initializeChartEvents() {
  chartElements.incomeExpenseChart.addEventListener("click", function () {
    renderExpandedChart(
      incomeExpenseChart,
  ...
  chartElements.categoryChart.addEventListener("click", function () {
    renderExpandedChart(
      categoryChart,
  ...
  chartElements.monthlyChart.addEventListener("click", function () {
    renderExpandedChart(
      monthlyChart,
  ...
  chartModalElements.closeButton.addEventListener(
    "click",

    closeChartModal,
  );

  chartModalElements.closeFooterButton.addEventListener(
    "click",

    closeChartModal,
  );

  chartModalElements.downloadButton.addEventListener(
    "click",

    downloadExpandedChart,
  );

  chartModalElements.overlay.addEventListener(
    "click",

    function (event) {
      if (event.target === chartModalElements.overlay) {
        closeChartModal();
      }
    },
  );
